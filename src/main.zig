...
pub fn main() !void {
    // Read port from environment — no allocator needed
    const port: u16 = blk: {
        const env = std.posix.getenv("FUNCTIONS_CUSTOMHANDLER_PORT") orelse break :blk 8080;
...
fn handleConnection(conn: net.Server.Connection) !void {
    ...
    // Read and discard the HTTP request (headers + any body)
